...
public class GameManager : NetworkBehaviour
{
	...
	private IEnumerator Mulligan()
	{
		...
		yield return new WaitForSeconds(0.1f); // for buff's Start to run before next handcard
	...
	{
		if (Buff.PlayerHasBuff("Altered Expectations", t) != null)
		{
            StartCoroutine(Buff.PlayerHasBuff("Altered Expectations", t).Glow());
            if (phase == 0) count = 0;
			else count += 1;
		}
    ...
    public IEnumerator GainHandCard(Team t, int id, FinalStats fs = null, bool conjured = true, bool animation = true)
	{
		...
		if (team == t)
		{
			if (handCards.childCount >= 10 || Buff.PlayerHasBuff("Suffocating Limits", GetOpponent(t)) != null && handCards.childCount >= 8)
			{
				StartCoroutine(Buff.PlayerHasBuff("Suffocating Limits", GetOpponent(t)).Glow());
				yield break;
			}
		...
		{
			if (opponentHandCards.childCount >= 10 || Buff.PlayerHasBuff("Suffocating Limits", GetOpponent(t)) != null && opponentHandCards.childCount >= 8)
			{
				StartCoroutine(Buff.PlayerHasBuff("Suffocating Limits", GetOpponent(t)).Glow());
                yield break;
			}
    ...
    {
		if (timeOut) if (Buff.PlayerHasBuff("Thin Patience", GetOpponent(sourceTeam)) != null)
			{
				StartCoroutine(Buff.PlayerHasBuff("Thin Patience", GetOpponent(sourceTeam)).Glow());
                StartCoroutine(GetTeamHero(sourceTeam).ReceiveDamage(5, null, true));
			}
    ...
    private IEnumerator Combat()
    {
        ...
        turn += 1;
		if (Buff.PlayerHasBuff("Nervous Laughter", team) == null) remaining = 0;
		else StartCoroutine(Buff.PlayerHasBuff("Nervous Laughter", team).Glow());
		if (Buff.PlayerHasBuff("Nervous Laughter", GetOpponent(team)) == null) opponentRemaining = 0;
		else StartCoroutine(Buff.PlayerHasBuff("Nervous Laughter", GetOpponent(team)).Glow());
	...
	{
		if (change > 0 && generated && Buff.PlayerHasBuff("Money Hungry", team) != null)
		{
			StartCoroutine(Buff.PlayerHasBuff("Money Hungry", team).Glow());
            change += 0.5f;
		}
