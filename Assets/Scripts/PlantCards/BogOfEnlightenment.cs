...
public class BogOfEnlightenment : Card
{
    ...
    {
        if (played.type == Type.Unit && played.team == team && played.col == col && played.amphibious) played.ChangeStats(2, 0);
        if (played.type == Type.Unit && played.team == GetOpponent(team) && played.col == col && !played.amphibious) played.ChangeStats(-2, 0);
    }

    protected override IEnumerator OnCardMoved(Card moved)
    {
        if (moved.oldCol == col && moved.col != col && moved.team == team && moved.amphibious) moved.ChangeStats(-2, 0);
        if (moved.oldCol == col && moved.col != col && moved.team == GetOpponent(team) && !moved.amphibious) moved.ChangeStats(2, 0);
